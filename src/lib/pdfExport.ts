 ...
 * Uses jsPDF with html rendering for CJK support (leverages browser's font rendering).
 */
...
export async function generateTravelBooklet(data: ExportData): Promise<void> {
  ...
  if (flights.length > 0) {
    ...
    autoTable(doc, {
      startY: y,
      head: [['Flight No', 'Airline', 'Route', 'Date', 'Time', 'Status']],
      body: flightRows,
  ...
  if (accommodations.length > 0) {
    ...
    autoTable(doc, {
      startY: y,
      head: [['Name', 'Address', 'Check-in', 'Check-out', 'Price']],
      body: accRows,
      margin: { left: margin, right: margin },
      styles: { fontSize: 8, cellPadding: 2 },
      headStyles: { fillColor: [30, 64, 175], textColor: [255, 255, 255] },
  ...
  doc.text(`Total: ${formatAmount(totalBase, trip.Base_Currency)} | ${expenses.length} items`, margin, y + 5);
  y += 12;
  ...
  autoTable(doc, {
    startY: y,
    head: [['Date', 'Category', 'Note', 'Amount', `${trip.Base_Currency}`, 'Payer', 'Splitters']],
    body: expenseRows,
    margin: { left: margin, right: margin },
  ...
  if (settlement && settlement.settlements && settlement.settlements.length > 0) {
    ...
    doc.setFontSize(11);
    doc.setTextColor(50, 50, 50);
    doc.text('Suggested Transfers', margin, y);
    y += 6;
