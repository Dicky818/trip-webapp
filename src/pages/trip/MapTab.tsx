...
export default function MapTab({ items, selectedDay, onDayChange, tripDays }: Props) {
  ...
  return (
    <div className="flex flex-col" style={{ minHeight: '600px' }}>
      {/* Day selector bar */}
      <div className="flex flex-shrink-0 items-center gap-2 border-b border-[#e3ddcf] bg-[#f5f2e8] px-4 py-2">
        <button onClick={handlePrevDay} disabled={viewDay === ALL_DAYS}
          className="rounded-lg p-1.5 text-slate-400 transition-colors hover:bg-white hover:text-[#111111] disabled:opacity-30">
          <ChevronLeft size={18} />
        </button>
        <div className="flex gap-1 flex-1 overflow-x-auto scrollbar-hide">
          ...
          <button
            ...
            className={`flex-shrink-0 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors whitespace-nowrap
              ${viewDay === ALL_DAYS ? 'bg-[#111111] text-[#ffc91a] shadow-sm' : 'text-slate-500 hover:bg-white hover:text-[#111111]'}`}>
            All
          ...
          {tripDays.map(({ day, date }) => {
            ...
            return (
              <button key={day} onClick={() => handleDayButtonClick(day)}
                className={`flex-shrink-0 rounded-lg border-2 px-3 py-1.5 text-xs font-medium transition-colors whitespace-nowrap
                  ${isSelected ? 'border-[#111111] bg-[#111111] text-[#ffc91a] shadow-sm' : 'border-transparent text-slate-500 hover:bg-white hover:text-[#111111]'}`}>
                <span className="flex items-center gap-1">
                  <span className="h-2 w-2 flex-shrink-0 rounded-full" style={{ backgroundColor: isSelected ? '#ffc91a' : color }} />
                  Day {day}
                </span>
      ...
      {viewDay === ALL_DAYS && tripDays.length > 0 && (
        <div className="flex-shrink-0 border-b border-[#e3ddcf] bg-[#faf8f2] px-4 py-2">
          <div className="flex flex-wrap gap-2">
            {tripDays.map(({ day, date }) => (
              <button key={day} onClick={() => handleDayButtonClick(day)}
                className="flex items-center gap-1 rounded-md px-1 py-0.5 text-xs text-slate-600 transition-colors hover:bg-white hover:text-[#111111]">
      ...
      <div className="relative flex-shrink-0" style={{ height: '280px' }}>
        ...
        {!mapsReady && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-[#f5f2e8]">
        ...
        {geocoding && (
          <div className="absolute top-2 right-2 z-[100] bg-white rounded-lg shadow px-3 py-1.5 flex items-center gap-2 text-xs text-slate-600">
            <Loader2 size={13} className="animate-spin text-[#111111]" />
            正在定位景點...
          </div>
        )}
