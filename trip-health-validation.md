...
The first browser request after the current-day Trip Pass refinement still loaded the earlier `HomePage-a8zwOw_i.js` module and therefore showed the original Day 1 next-stop label. The health card itself was correct. A clean production cache and current entry-bundle check are required before accepting the Trip Pass refinement as published.

The GitHub Pages branch confirms the current `index-BJ-l4DQ0.js` build is published. Clearing the isolated review browser’s Service Worker and caches removed its active application session, so the fresh browser context showed the no-trip portal and could not independently re-read the authenticated Trip Pass. The code path was rebuilt and type-checked after the current-day selection change; the signed-in production visual recheck can occur when an authenticated session is available again.
